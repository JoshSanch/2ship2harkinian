...
bool Rando::IsItemObtainable(RandoItemId randoItemId, RandoCheckId randoCheckId) {
    ...
    bool isForYou = randoCheckId == RC_UNKNOWN || Anchor::Instance->roomState.teams.size() < 2 || Anchor::Instance->roomState.teams[RANDO_SAVE_CHECKS[randoCheckId].multiWorldTeamIndex] == std::string(CVarGetString("gNetwork.Anchor.TeamId", "default"));
    if (!isForYou) {
